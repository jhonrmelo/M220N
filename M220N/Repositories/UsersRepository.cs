        ...
        */
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var sessionFilter = Builders<Session>.Filter.Eq(x => x.UserId, user.Email);

                var updateSessionFilter = Builders<Session>.Update.Set(x => x.Jwt, user.AuthToken).Set(x => x.UserId, user.Email);

                await _sessionsCollection.UpdateOneAsync(sessionFilter, updateSessionFilter, new UpdateOptions() { IsUpsert = true });
        ...
        public async Task<UserResponse> LogoutUserAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            await _sessionsCollection.DeleteOneAsync(Builders<Session>.Filter.Eq(x => x.UserId, email), cancellationToken);
        ...
        public async Task<Session> GetUserSessionAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            return await _sessionsCollection.Find(Builders<Session>.Filter.Eq(x => x.UserId, email)).FirstOrDefaultAsync(cancellationToken);
        }

...
namespace M220N.Repositories
{
    public class MoviesRepository
    {
        ...
        public async Task<Movie> GetMovieAsync(string movieId, CancellationToken cancellationToken = default)
        {
            try
            {
                return await _moviesCollection.Aggregate()
                    ...
                    .Lookup(_commentsCollection, x => x.Id, c => c.MovieId, (Movie m) => m.Comments)
            ...
            )
        {
            ...
            var projection = Builders<Movie>.Projection.Include(m => m.Id)
                                                       .Include(m => m.Title);

            return await _moviesCollection.Find(filter).SortByDescending(m => m.Title).Project<MovieByCountryProjection>(projection).ToListAsync();
        }
